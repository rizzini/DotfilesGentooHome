...
if [[ ! "$files" ==  *'1.srt'* && "$files" ==  *'srt'* ]]; then
    ...
    read -r numero_srt;
    /usr/bin/find "$PWD" -iname "*.srt" -type f | /usr/bin/sort | /usr/bin/gawk 'BEGIN{ a="'"$numero_srt"'" }{ printf "/usr/bin/mv '\''%s'\'' %01d.srt\n", $0, a++ }';
...
if [[ ! "$files" == *'1.mkv'* && "$files" ==  *'mkv'* ]]; then
    ...
    /bin/echo "Começar a partir de qual número?";
    read -r numero_mkv;
    /usr/bin/find "$PWD" -iname "*.mkv" -type f | /usr/bin/sort | /usr/bin/gawk 'BEGIN{ a="'"$numero_mkv"'" }{ printf "/usr/bin/mv '\''%s'\'' %01d.mkv\n", $0, a++ }';
    ...
    /bin/echo "Começar a partir de qual número?";
    read -r numero_mp4;
    /usr/bin/find "$PWD" -iname "*.mp4" -type f | /usr/bin/sort | /usr/bin/gawk 'BEGIN{ a="'"$numero_mp4"'" }{ printf "/usr/bin/mv '\''%s'\'' %01d.mp4\n", $0, a++ }';
    /bin/echo "Renomear vídeos? S ou N: ";
    read -r pergunta_mp4;
